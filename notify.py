...
def build_email_body(date_str: str, weekday: int, schedule_pages: list, novel_pages: list) -> str:
    ...
    if schedule_only:
        ...
        for p in schedule_only:
            time_str = get_time(p)
            title = html.escape(get_title(p))
            lines.append(f"<li>{time_str} {title}</li>" if time_str else f"<li>{title}</li>")
        lines.append("</ul>")
    else:
        lines.append("<ul>")
    ...
    if todo_pages:
        ...
        lines.append("<ul>")
        for p in todo_pages:
            kind = html.escape(get_select(p, "종류"))
            time_str = get_time(p)
            title = html.escape(get_title(p))
    ...
    if novel_pages:
        ...
        for p in novel_pages:
            work = html.escape(get_select(p, "작품"))
            kind = html.escape(get_select(p, "유형"))
            time_str = get_time(p)
            title = html.escape(get_title(p))
            body_text = f"{time_str} {title}" if time_str else title
            lines.append(f"<li>[{work}] {kind} - {body_text}</li>")
        lines.append("</ul>")
    else:
