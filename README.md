...
- `docs/` : GitHub Pages로 서비스되는 PWA(휴대폰에 설치하는 앱) 소스. 빌드 과정 없이 정적 파일 그대로 배포된다.
  ...
  - `app.js` : `today.json` 로딩, 취침 기록·조정 제안·루틴 편집기·설정 등 화면과 상태를 담당
  - `manifest.json`, `service-worker.js`, `icon-*.png` : 홈 화면 설치 + 오프라인 캐싱을 위한 PWA 필수 파일
  - `today.json` : `notify.py`가 매일 아침 자동으로 덮어쓰는 데이터(공휴일/특별 일정/할일/소설 + 오늘 알림 발송 여부)
- `tools/make_icons.py` : 앱 아이콘(PNG)을 생성하는 로컬 전용 스크립트. 결과물만 커밋하며 CI에서는 실행하지 않는다.
- `tests/routine.test.mjs` : `node --test tests/routine.test.mjs`로 실행하는 루틴 계산 단위 테스트
...
- 노션 [일정] DB에 "종류"가 "일정"인 항목을 등록하면, 그날은 이 루틴 대신 그 항목들이 그대로 표시됩니다. "할일"/"과제"는 루틴(또는 특별 일정)과 별도로 항상 함께 표시됩니다.
