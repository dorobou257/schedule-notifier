...
- 기본 루틴은 기상 08:00 / 취침 00:30을 기준으로 아침·점심·저녁 식사 뒤에 집필 3블록(각 2시간), 저녁 식사 전 작업, 밤에 운동(월/수/금)이 배치되어 있습니다. 정확한 정의는 `docs/routine.js`의 `BASE_BLOCKS` 참고.
- 앱 하단의 "취침" 버튼을 누르면 그 시각 + 목표 수면 시간(기본 7.5시간)으로 다음 기상 시각을 예상합니다. 실제로 다른 시각에 일어났다면 "아침 보정"으로 직접 입력할 수 있습니다.
- 기상이 늦어지면 점심(13:00 고정)까지 남은 시간에 맞춰 아침 식사부터 자동으로 없어지고, 그래도 부족하면 설정에서 정한 우선순위(기본: 여유 → 운동 → 작업 → 집필) 순으로 블록이 줄어듭니다. 점심 이후 일정과 취침 시각은 건드리지 않습니다.
